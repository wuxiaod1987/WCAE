...
namespace TriangleNet.IO
{
    ...
    public static class FileWriter
    {
        ...
        /// <summary>
        /// Transform vertics and triangles into personal structure Node and Element, then output.
        /// </summary>
        public static void Trans_Mesh(Mesh mesh, ref Node[] Ns, ref Element[] Es)
        {
            /////因为经过了Refine有许多空余节点编号，扣除空余节点
            int QuanN = mesh.vertices.Count;
            Ns = new Node[QuanN + 1];
            int[] Ns_Trans = new int[0];
            int Num = 0;

            foreach (var node in mesh.vertices.Values)
            {
                Num++;
                Array.Resize<int>(ref Ns_Trans, node.id + 1);
                Ns_Trans[node.id] = Num;
                Ns[Num] = new Node(node.X, node.Y, (int)node.mark);
            }
            ////////////////////////////////////////////
            int QuanE = mesh.triangles.Count;
            Es = new Element[QuanE];
            int N1, N2, N3;
            Num = 0;
            ...
            foreach (var item in mesh.triangles.Values)
            {
                ...
                N2 = Ns_Trans[N2];
                N3 = Ns_Trans[N3];
                Es[Num] = new Element(Num, N1, N2, N3, (float)item.Area);

                EsB_Adding(ref Ns[N1], Num);
                EsB_Adding(ref Ns[N2], Num);
                EsB_Adding(ref Ns[N3], Num);
                Num++;
            }
        }
