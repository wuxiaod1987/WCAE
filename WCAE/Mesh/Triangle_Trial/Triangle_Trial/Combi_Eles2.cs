...
{
    public static class Combi_Eles2
    {
        ...
        static public void Elements_Combine(ref Node[] Ns, ref Element[] Es, double Angle_Max)
        {
            for (int i = 0; i < Es.Length; i++)
                Element_Combine(ref Ns, ref Es, i, Angle_Max);
        }
        #endregion
        ...
        static private int[] Element_Combine_ParaCompute(ref Node[] Ns, ref Element[] Es, int Num_E, double Angle_Max)
        {
            ...
            Nos[2] = Fourth_Node_Find(N1, N3, N2, Num_E, ref Ns, ref Es);     /////N1,N3情况
        ...
        static private int[] Fourth_Node_Find(int N1, int N2, int N3, int Em, ref Node[] Ns, ref Element[] Es)
        {
            ...
            if (Es[E].N1 != N1 && Es[E].N1 != N2)
                return new int[] { E, Es[E].N1, N1, N3, N2 };
            if (Es[E].N2 != N1 && Es[E].N2 != N2)
                return new int[] { E, Es[E].N2, N1, N3, N2 };
            if (Es[E].N3 != N1 && Es[E].N3 != N2)
                return new int[] { E, Es[E].N3, N1, N3, N2 };
            return new int[] { 0, 0 };
        }
        #endregion
        ...
        static private void Output(string Path, string Name, ref Node[] Ns, ref Element[] Es)
        {
            StreamWriter sw = new StreamWriter(Path + Name + ".n");
            string t;
            for (int i = 1; i < Ns.Length; i++)
            {
                t = Convert.ToString(Ns[i].x) + "," + Convert.ToString(Ns[i].y);
                t = Convert.ToString(i) + "	" + t;
                sw.WriteLine(t);
            }
            sw.Close();

            sw = new StreamWriter(Path + Name + ".e");
            for (int i = 1; i < Es.Length; i++)
            {
                ...
                t = "";
                if (Es[i].Kind == 2)
                    t = Convert.ToString(Es[i].Kind) + "	" + Convert.ToString(Es[i].N1) + "," + Convert.ToString(Es[i].N2);
                if (Es[i].Kind == 3)
                    t = Convert.ToString(Es[i].Kind) + "	" + Convert.ToString(Es[i].N1) + "," + Convert.ToString(Es[i].N2) + "," + Convert.ToString(Es[i].N3);
                if (Es[i].Kind == 4)
                ...
                t = Convert.ToString(i) + "	" + t + "	" + Convert.ToString(i);
                sw.WriteLine(t);
            }
            sw.Close();
        }
